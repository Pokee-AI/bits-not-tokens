...
cat scripts/queue_v4.txt scripts/queue_v4_raw.txt > scripts/queue_v4_all.txt
nohup scripts/run_queue.sh scripts/queue_v4_all.txt > logs/queue_v4.log 2>&1 &
sleep 2
echo "relaunched $(wc -l < scripts/queue_v4_all.txt) jobs"
